...
class Decoder(nn.Module):
    def __init__(self, args):
        ...
        elif args["task"] == "graph_binary":  # intent_success, dest_{success/scoring/conceding}
            out_dim = 1 if args["target"].endswith("success") else 2
            self.graphwise_mlp = nn.Sequential(
                nn.Linear(graph_in_dim, 32),
                nn.ReLU(),
                nn.Linear(32, 8),
    ...
    def forward(
        ...
        if self.args["task"].startswith("node"):  # node_selection, node_binary
            ...
            elif self.args["target"] == "intent_success":
                return self.graphwise_mlp(graph_embeddings).squeeze(-1)  # [B,]

            elif self.args["target"] in ["intent_scoring", "intent_conceding"]:
                return self.graphwise_mlp(graph_embeddings)  # [B, 2]
